...
- **Containerized Testing**: A ready-to-use Docker environment leveraging `docker-compose` spins up the Next.js portal alongside an `atmoz/sftp` testing server with a single command.
...
This command will:
1. Build the multi-stage Next.js production `standalone` Node.js image.
2. Spin up an `atmoz/sftp` test container mapped with default test credentials (`testuser:testpass`).
3. Boot the Next.js API server (waiting on the SFTP container's health checks) and expose the interface on `http://localhost:3000`.
